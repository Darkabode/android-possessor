...
public class NativeToJsMessageQueue
{
    ...
    public NativeToJsMessageQueue(UIWebView webView)
    {
     ...
     * Add a JavaScript statement to the list.
     */
    public void addPluginResult(ModuleResult result, String callbackId)
    {
        if (callbackId == null) {
            return;
        }
        // Don't send anything if there is no result and there is no need to clear the callbacks.
        boolean noResult = result.getStatus() == ModuleResult.Status.NO_RESULT.ordinal();
    ...
    private class OnlineEventsBridgeMode extends BridgeMode
    {
        ...
        void reset()
        {
            try {
                LockActivity a = LockActivity.getInstance();
                if (a != null) {
                    a.runOnUiThread(resetNetworkRunnable);
                }
            }
            catch (Exception e) {
                e.printStackTrace();
        ...
        void onNativeToJsMessageAvailable()
        {
            try {
                LockActivity a = LockActivity.getInstance();
                if (a != null) {
                    a.runOnUiThread(toggleNetworkRunnable);
                }
            }
            catch (Exception e) {
                e.printStackTrace();
    ...
    private static class JsMessage
    {
        ...
        final ModuleResult pluginResult;
        ...
        JsMessage(ModuleResult pluginResult, String callbackId)
        {
        ...
        static int calculateEncodedLengthHelper(ModuleResult pluginResult) {
            switch (pluginResult.getMessageType()) {
                case ModuleResult.MESSAGE_TYPE_BOOLEAN: // f or t
                case ModuleResult.MESSAGE_TYPE_NULL: // N
                    return 1;
                case ModuleResult.MESSAGE_TYPE_NUMBER: // n
                    return 1 + pluginResult.getMessage().length();
                case ModuleResult.MESSAGE_TYPE_STRING: // s
                    return 1 + pluginResult.getStrMessage().length();
                case ModuleResult.MESSAGE_TYPE_BINARYSTRING:
                    return 1 + pluginResult.getMessage().length();
                case ModuleResult.MESSAGE_TYPE_ARRAYBUFFER:
                    return 1 + pluginResult.getMessage().length();
                case ModuleResult.MESSAGE_TYPE_MULTIPART:
                    int ret = 1;
                    ...
                    return ret;
                case ModuleResult.MESSAGE_TYPE_JSON:
                default:
        ...
        static void encodeAsMessageHelper(StringBuilder sb, ModuleResult pluginResult) {
            switch (pluginResult.getMessageType()) {
                case ModuleResult.MESSAGE_TYPE_BOOLEAN:
                ...
                case ModuleResult.MESSAGE_TYPE_STRING: // s
                    ...
                    break;
                case ModuleResult.MESSAGE_TYPE_BINARYSTRING: // S
                    sb.append('S');
                    sb.append(pluginResult.getMessage());
                    break;
                case ModuleResult.MESSAGE_TYPE_ARRAYBUFFER: // A
                    sb.append('A');
                    sb.append(pluginResult.getMessage());
                    break;
                case ModuleResult.MESSAGE_TYPE_MULTIPART:
                    sb.append('M');
                    for (int i = 0; i < pluginResult.getMultipartMessagesSize(); i++) {
                        ModuleResult multipartMessage = pluginResult.getMultipartMessage(i);
        ...
        void encodeAsMessage(StringBuilder sb) {
            ...
            boolean noResult = status == ModuleResult.Status.NO_RESULT.ordinal();
            boolean resultOk = status == ModuleResult.Status.OK.ordinal();
        ...
        void encodeAsJsMessage(StringBuilder sb) {
            if (pluginResult == null) {
                ...
                int status = pluginResult.getStatus();
                boolean success = (status == ModuleResult.Status.OK.ordinal()) || (status == ModuleResult.Status.NO_RESULT.ordinal());
